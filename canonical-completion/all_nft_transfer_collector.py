...
CHAIN_ID = 4663
USER_AGENT = "RHC-All-NFT-Transfers/1.0"
...
TARGETS = {
    "erc721": {"topics": [ERC721_TRANSFER]},
    "erc1155_single": {"topics": [keccak256("TransferSingle(address,address,address,uint256,uint256)")]},
    "erc1155_batch": {"topics": [keccak256("TransferBatch(address,address,address,uint256[],uint256[])")]},
}
...
class RpcClient:
    def __init__(self, min_interval: float = 0.95):
...
def recursive_logs(client: RpcClient, topics: list[Any], head: int, chunk: int = 150_000) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    while queue:
        ...
        try:
            ...
            completed += 1
            if completed % 25 == 0:
                print({"completed_ranges": completed, "queued": len(queue), "raw_events": len(rows)}, flush=True)
        except Exception as exc:
            if left < right and depth < 30:
                middle = (left + right) // 2
...
def uint_array(words: list[str], offset_bytes: int) -> list[int]:
    index = offset_bytes // 32
    count = int(words[index], 16)
    return [int(words[index + 1 + i], 16) for i in range(count)]


def decode(target: str, row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if target == "erc721":
        # ERC-20 shares the Transfer signature but has only 3 topics.
        if len(topics) != 4:
            return None
        base.update({
        ...
        if len(ids) != len(amounts):
            raise ValueError("ERC1155 batch mismatch")
        base.update({
            "operator": topic_address(topics[1]),
            "from_address": topic_address(topics[2]),
            "to_address": topic_address(topics[3]),
...
def batch_fetch(client: RpcClient, method: str, values: list[Any], size: int) -> dict[Any, Any]:
    ...
    for start in range(0, len(values), size):
        ...
        if start % (size * 10) == 0:
            print({"method": method, "done": min(start + len(chunk), len(values)), "total": len(values)}, flush=True)
    return output
...
def main() -> None:
    ...
    raw, unresolved = recursive_logs(client, TARGETS[args.target]["topics"], head)
    ...
    for row in decoded:
        ...
        row["event_kind"] = "MINT" if row.get("from_address") == ZERO else "BURN" if row.get("to_address") == ZERO else "TRANSFER"
    write_jsonl(out / "raw_transfer_logs.jsonl", logs)
    write_jsonl(out / "decoded_nft_transfers.jsonl", decoded)
    write_csv(out / "decoded_nft_transfers.csv", decoded)
    ...
    for row in decoded:
        item = contract_summary.setdefault(row["contract_address"], {
            "contract_address": row["contract_address"],
            "token_standard": row["token_standard"],
            "first_event_block": row["block_number"],
            "last_event_block": row["block_number"],
            "event_rows": 0,
            "mint_rows": 0,
            "burn_rows": 0,
            "transfer_rows": 0,
        ...
        item["first_event_block"] = min(item["first_event_block"], row["block_number"])
        item["last_event_block"] = max(item["last_event_block"], row["block_number"])
        item["event_rows"] += 1
        item["mint_rows"] += row["event_kind"] == "MINT"
        item["burn_rows"] += row["event_kind"] == "BURN"
        item["transfer_rows"] += row["event_kind"] == "TRANSFER"
        item["transactions"].add(row["transaction_hash"])
        if row.get("from_address") and row["from_address"] != ZERO:
            item["participants"].add(row["from_address"])
        if row.get("to_address") and row["to_address"] != ZERO:
            item["participants"].add(row["to_address"])
    summary_rows = []
    for value in contract_summary.values():
        summary_rows.append({
            **{key: item for key, item in value.items() if key not in ("transactions", "participants")},
            "unique_transactions": len(value["transactions"]),
            "unique_participants": len(value["participants"]),
        })
    ...
    validation = {
        ...
        "ignored_non_nft_transfer_rows": ignored_non_nft,
        "canonical_nft_transfer_rows": len(decoded),
        "unique_contracts": len(summary_rows),
