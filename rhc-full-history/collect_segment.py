...
USER_AGENT = "RHC-NFT-Full-History/1.0 (read-only research)"
...
def pace(min_interval: float = 0.23) -> None:
...
def request_json(url: str, attempts: int = 12) -> dict[str, Any]:
    ...
    for attempt in range(attempts):
        ...
        try:
            ...
            if exc.code == 429:
                ...
                delay = float(retry) if retry and retry.replace(".", "", 1).isdigit() else min(120, 8 + attempt * 8 + random.random() * 4)
                time.sleep(delay)
                continue
...
def collect(target: str, segment_index: int, segment_count: int, end_block: int, out: Path, workers: int) -> None:
    start_block = (end_block + 1) * segment_index // segment_count
    segment_end = ((end_block + 1) * (segment_index + 1) // segment_count) - 1
    out.mkdir(parents=True, exist_ok=True)
    pending: list[tuple[int, int]] = [(start_block, segment_end)]
...
def main() -> None:
    ...
    collect(args.target, args.segment_index, args.segment_count, args.end_block, args.out, max(1, args.workers))
