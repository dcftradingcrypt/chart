...
BASES=['https://explorer.hoodmarketcap.com/api','https://robinhoodchain.blockscout.com/api']
TARGETS={
 'seadrop':{'address':'0x00005ea00ac477b1030ce78506496e8c2de24bf5','topic0':'0xe90cf9cc0a552cf52ea6ff74ece0f1c8ae8cc9ad630d3181f55ac43ca076b7d6'},
 'seaport':{'address':'0x0000000000000068f116a894984e2db1123eb395','topic0':'0x9d9af8e38d66c62e2c12f0225249fd9d721c54b83f48d9352c97c6cacdcb6f31'}}
UA='RHC-NFT-Full-History/2.1 (read-only)';L=threading.Lock();next_at=0.;calls=0;backoffs=0
...
def one(url,attempts=2):
 ...
 for i in range(attempts):
  ...
  try:
   ...
   with urllib.request.urlopen(q,timeout=15) as r:raw=r.read()
...
def query(target,a,b):
 s=TARGETS[target];notes=[]
 p={'module':'logs','action':'getLogs','fromBlock':a,'toBlock':b,'address':s['address'],'topic0':s['topic0']}
 for base in BASES:
...
def collect(target,idx,count,end,out,workers,window):
 ...
 pending=[(a,min(a+window-1,stop)) for a in range(start,stop+1,window)]
 accepted=[];ranges=[];errors=[]
 while pending:
