...
SOURCE_BRANCH = "chatgpt/rhc-token-transfer-aggregate-20260829"
SOURCE_WORKFLOW = "RHC aggregate complete token-level NFT transfer histories"
SOURCE_ARTIFACT = "rhc-token-transfer-aggregate"
RPC = "https://rpc.mainnet.chain.robinhood.com"
BLOCKSCOUT = "https://robinhoodchain.blockscout.com/api/v2"
ZERO_TOPIC = "0x" + ("0" * 64)
USER_AGENT = "RHC-Secondary-Tx-Enrichment/1.0"
SHARD_COUNT = 16
...
def wait_source(timeout: int = 19_800) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        ...
        if not rows:
            print("waiting for token-transfer aggregate", flush=True)
            time.sleep(30)
            continue
        run = rows[0]
        print(json.dumps({
        ...
        if run.get("status") == "completed":
            if run.get("conclusion") != "success":
                raise RuntimeError(f"token-transfer aggregate failed: {run.get('html_url')}")
            return run
        time.sleep(45)
    raise TimeoutError("token-transfer aggregate did not finish")
...
def fetch_source(out: Path) -> tuple[dict[str, Any], Path]:
    ...
    if len(matches) != 1:
        raise RuntimeError(f"token-transfer aggregate artifact count={len(matches)}")
    artifact = matches[0]
    zpath = out / "source" / "token-transfers.zip"
    download(artifact["archive_download_url"], zpath)
    extract = out / "source" / "token-transfers"
...
def transfer_parties(row: dict[str, Any]) -> tuple[str, str]:
    raw = row["raw"]
    ...
    if event_name == "ERC721_TRANSFER":
        ...
        return topic_address(topics[1]), topic_address(topics[2])
    if event_name in {"ERC1155_TRANSFER_SINGLE", "ERC1155_TRANSFER_BATCH"}:
        if len(topics) != 4:
            raise RuntimeError(f"ERC1155 topic count {len(topics)}")
        return topic_address(topics[2]), topic_address(topics[3])
    raise RuntimeError(f"unknown event_name {event_name}")
...
class Client:
    ...
    def rpc_batch(self, calls: list[tuple[str, list[Any]]]) -> list[Any]:
        ...
        for index in range(len(calls)):
            row = by_id.get(index)
            if row is None:
                output.append({"__error__": "missing response"})
            elif "error" in row:
...
def paginate_blockscout(client: Client, path: str) -> list[dict[str, Any]]:
    ...
    for _ in range(10_000):
        ...
        if not isinstance(payload, dict):
            raise RuntimeError(f"invalid paginated response for {path}")
...
def main() -> None:
    ...
    source_status = json.loads(only_file(root, "AGGREGATE_STATUS.json").read_text(encoding="utf-8"))
    if source_status.get("status") != "PASS":
        raise RuntimeError("token-transfer aggregate is not PASS")

    transfer_rows = list(load_jsonl_gz(only_file(root, "contract_transfer_events.jsonl.gz")))
    tx_meta: dict[str, dict[str, Any]] = {}
    for row in transfer_rows:
        from_address, to_address = transfer_parties(row)
        if from_address == "0x" + ("0" * 40) or to_address == "0x" + ("0" * 40):
            continue
        raw = row["raw"]
        tx_hash = str(raw.get("transactionHash") or raw.get("transaction_hash") or "").lower()
        block = intish(raw.get("blockNumber") or raw.get("block_number"))
        meta = tx_meta.setdefault(tx_hash, {
        ...
        if meta["block_number"] != block:
            raise RuntimeError(f"transaction in multiple blocks: {tx_hash}")
        meta["transfer_event_count"] += 1
        meta["contracts"].add(row["contract"].lower())
        meta["from_addresses"].add(from_address)
        meta["to_addresses"].add(to_address)

    selected_hashes = sorted(tx_hash for tx_hash in tx_meta if shard_for(tx_hash) == args.shard)
    input_snapshot = {
        "shard": args.shard,
        "shard_count": SHARD_COUNT,
        "source": source_record,
        "transaction_count": len(selected_hashes),
        "transactions": [
            {
                **tx_meta[tx_hash],
                "contracts": sorted(tx_meta[tx_hash]["contracts"]),
                "from_addresses": sorted(tx_meta[tx_hash]["from_addresses"]),
                "to_addresses": sorted(tx_meta[tx_hash]["to_addresses"]),
            }
    ...
    for tx_chunk in chunks(selected_hashes, 40):
        calls = []
        keys = []
        ...
        try:
            ...
            errors.append({"kind": "rpc_batch", "values": tx_chunk, "error": repr(exc)})
            continue
    ...
    block_numbers = sorted({tx_meta[tx_hash]["block_number"] for tx_hash in selected_hashes})
    for block_chunk in chunks([str(value) for value in block_numbers], 80):
        calls = [("eth_getBlockByNumber", [hex(int(value)), False]) for value in block_chunk]
        try:
            results = client.rpc_batch(calls)
        except Exception as exc:
            errors.append({"kind": "block_batch", "values": block_chunk, "error": repr(exc)})
            continue
        for value, result in zip(block_chunk, results):
            block = int(value)
            if result is None or isinstance(result, dict) and "__error__" in result:
                try:
                    fallback = client.blockscout(f"blocks/{block}")
                    block_rows[block] = {"block_number": block, "source": "BLOCKSCOUT_FALLBACK", "raw": fallback}
                except Exception as fallback_exc:
                    errors.append({"kind": "block", "block_number": block, "fallback_error": repr(fallback_exc)})
            else:
                block_rows[block] = {"block_number": block, "source": "OFFICIAL_RPC", "raw": result}

    for tx_hash in selected_hashes:
        try:
            items = paginate_blockscout(client, f"transactions/{tx_hash}/internal-transactions")
            internal_rows.append({"transaction_hash": tx_hash, "source": "BLOCKSCOUT_V2", "items": items})
        except Exception as exc:
            errors.append({"kind": "internal_transactions", "transaction_hash": tx_hash, "error": repr(exc)})

    tx_count = write_jsonl_gz(out / "transactions.jsonl.gz", tx_rows)
    receipt_count = write_jsonl_gz(out / "receipts.jsonl.gz", receipt_rows)
    block_count = write_jsonl_gz(out / "blocks.jsonl.gz", block_rows.values())
    ...
    fetched_receipt = {row["transaction_hash"] for row in receipt_rows}
    fetched_internal = {row["transaction_hash"] for row in internal_rows}
    failures = []
    for code, expected, actual in (
        ("MISSING_TRANSACTIONS", set(selected_hashes), fetched_tx),
        ("MISSING_RECEIPTS", set(selected_hashes), fetched_receipt),
        ("MISSING_INTERNALS", set(selected_hashes), fetched_internal),
        ("MISSING_BLOCKS", set(block_numbers), set(block_rows)),
    ):
        missing = sorted(expected - actual)
        if missing:
            failures.append({"code": code, "count": len(missing), "sample": missing[:25]})
    if errors:
        failures.append({"code": "ENRICHMENT_ERRORS", "count": len(errors), "sample": errors[:25]})
    ...
    validation = {
        ...
        "shard_count": SHARD_COUNT,
        "source": source_record,
        "expected_transaction_count": len(selected_hashes),
        "expected_block_count": len(block_numbers),
        "fetched": {
