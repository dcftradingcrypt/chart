...
UA = "RHC-Secondary-Transfer-Enrichment/1.0 read-only"
    ...
    for base in BLOCKSCOUTS:
        try:
            ...
            if not isinstance(items, list):
                raise RuntimeError(f"invalid internal transaction payload: {payload!r}")
            return [item for item in items if isinstance(item, dict)], base, errors
        except Exception as exc:
...
def main() -> None:
    ...
    for start in range(0, len(selected), 20):
        ...
        try:
            ...
            errors.append({"stage": "rpc_batch", "transaction_hashes": chunk, "error": repr(exc)})
            continue
    ...
    for position, tx_hash in enumerate(selected, 1):
        ...
        native_internal: list[dict[str, Any]] = []
        for log in receipt.get("logs") or []:
            ...
            if len(topics) == 3 and topics[0] == TRANSFER_TOPIC:
                erc20_transfers.append({
                    ...
                    "amount_raw": str(intish(log.get("data"))),
        ...
        for item in internal:
            ...
            value = intish(raw_value)
            if value <= 0:
                continue
            from_value = item.get("from")
            to_value = item.get("to")
            if isinstance(from_value, dict):
                from_value = from_value.get("hash") or from_value.get("address")
            if isinstance(to_value, dict):
                to_value = to_value.get("hash") or to_value.get("address")
            native_internal.append({
                "from_address": str(from_value or "").lower(),
                "to_address": str(to_value or "").lower(),
                "value_wei": str(value),
        ...
        rows.append({
            ...
            "nft_secondary_transfer_count": len(by_tx[tx_hash]),
            "nft_secondary_transfers": by_tx[tx_hash],
            "erc20_transfer_logs": erc20_transfers,
    ...
    with gzip.open(out / "secondary_transfer_transactions.jsonl.gz", "wt", encoding="utf-8") as file:
        for row in rows:
    ...
    if missing:
        failures.append({"code": "SECONDARY_TRANSACTION_ENRICHMENT_MISSING", "count": len(missing), "sample": missing[:50]})
    if errors:
        failures.append({"code": "ENRICHMENT_ERRORS_PRESENT", "count": len(errors)})
    if any(row["receipt_status"] != 1 for row in rows):
        failures.append({"code": "NON_SUCCESS_RECEIPT_IN_SECONDARY_TRANSFER_INDEX"})
    report = {
        ...
        "decision_use": "NON_SEAPORT_SALE_AND_TOKEN_PROVENANCE_INPUT",
    }
